...
output_path="D:/297 Big data/BigDataProject" #Path to store output graphs and csv files
...
write.csv(trend_new, file=file.path(output_path, "tweets_topTrend.csv"), row.names=F)
...
searchTerm # To verify search term
searchTerm <- "Lou Richards" 
no_tweets <- 200 #to define the number of tweets
#Getting tweets
...
write.csv(clean_tweet, file=file.path(output_path, "tweets_cleansed.csv"), row.names=F)
...
png(paste(output_path,"wordCloud.png"), width=800, height = 600)
